...
function createTreeView(menu, currentNode) {
  ...
  // 자식 노드가 있을 때  - (1) 각각 체크박스를 보여주고 checkbox존재 (2) li 엘리먼트 아래에 자식 노드를 렌더링할 새로운 ul이 존재해야 함
  // 자식 노드가 없을 때  - (1) li 엘리먼트 안에 단순히 이름만 표시 
  for(let i=0; i< menu.length; i++) {
    const $li = document.createElement('li');
    if (menu[i].children) {
      const $input = document.createElement('input'); // 카테고리(음료, 음식, 굿즈, 카드) 엘리먼트 안에는 각각 자식 노드를 보여주고 감춰줄 checkbox가 존재해야 합니다
      $input.type = 'checkbox';
      const $span = document.createElement('span');
      $span.textContent = menu[i].name; // 음료, 음식, 굿즈, 카드 카테고리 이름(name)을 span 태그로 감싸야 합니다
      const $ul = document.createElement('ul');
      
      $li.append($input, $span, $ul);
      currentNode.append($li);

      createTreeView(menu[i].children, $ul); // 자식 노드가 있는 데이터의 경우, li 엘리먼트 아래에 자식 노드를 렌더링할 새로운 ul이 존재해야 합니다 (재귀함수)
    }else { // 자식 요소가 없는 객체
      $li.textContent = menu[i].name; // 자식 노드가 없는 데이터의 경우, li 엘리먼트 안에 단순히 이름(name)만 표시합니다
      currentNode.append($li); 
    }
  }
}
